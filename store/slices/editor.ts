...
import {Permission, TextEditStatus, UploadStatus} from "../../components/types/common/status";
...
const layoutSlice = createSlice({
	...
	reducers: {
		...
		addLayoutItem: (
			...
			if (state.layouts[0].sections[sectionIndex].items.length >= MAX_LAYOUT_VALUE) return;

			state.layouts[0].sections[sectionIndex].items.push(
					payload.newItem
			);
		},
		addImageToSection: (state, {payload}: PayloadAction<{ sectionKey: string }>) => {
			const section = state.layouts[0].sections.find(
					(section) => section.sectionKey === payload.sectionKey
			);
			if (!section) return;
		...
		deleteLayoutItem(state, {payload}: PayloadAction<{ sectionId: string; itemId: string }>) {
			const section = state.layouts[0].sections.find(
					(section) => section.sectionKey === payload.sectionId
			);
			if (!section) return;
			section.items = section.items.filter((item) => item.id !== payload.itemId);
		},
		updateImageUrl(
				state,
				{payload}: PayloadAction<{ sectionKey: string; itemId: string; imageUrl: string }>
		) {
			const section = state.layouts[0].sections.find(
					(section) => section.sectionKey === payload.sectionKey
			);
			...
			if (item?.type === "img") {
				item.componentProps = {...item.componentProps, ...{url: payload.imageUrl}} as ImageProps;
			}
		},
		updateTextContent(
				state,
				{payload}: PayloadAction<{ sectionKey: string; itemId: string; textContent: string }>
		) {
			const section = state.layouts[0].sections.find(
					(section) => section.sectionKey === payload.sectionKey
			);
			const item = section?.items.find((item) => item.id === payload.itemId);
			if (item?.type === "text") {
				item.componentProps.text = payload.textContent
			}
		},
		setLayoutData(
				state,
				{payload}: PayloadAction<{ layout: LayoutState }>
		) {
			state.layouts[0] = payload.layout;
		},
		setImageUploadStatus(
				state,
				{payload}: PayloadAction<{ itemKey: string; status: Partial<UploadStatus> }>
		) {
		...
		updateTextStyle: (
			...
			const section = state.layouts[0].sections.find(s => s.sectionKey === payload.sectionKey);
			const item = section?.items.find(i => i.id === payload.itemId);
			if (item?.type === "text") {
				const current = item.componentProps;
		...
		updateSectionBackground: (
			...
			const section = state.layouts[0].sections.find(s => s.sectionKey === payload.sectionKey);
			if (section) {
				section.backgroundStyle = {...section.backgroundStyle, ...payload.background};
			}
		},
		...
		setProjectPermission(state, action: PayloadAction<{ projectPermission: Permission }>) {
			state.projectPermission = action.payload.projectPermission;
		},
		setProjectPublishUrl(state, action: PayloadAction<{ projectPublishUrl: string | null }>) {
